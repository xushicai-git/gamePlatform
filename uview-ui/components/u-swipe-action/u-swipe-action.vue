	...
	<view class="">
		<movable-area class="u-swipe-action" :style="{ backgroundColor: bgColor }">
			...
			>
				...
				<view class="u-swipe-del" v-if="showBtn" @tap.stop="contentClick(index)" :style="[btnStyle(item.style)]" v-for="(item, index) in options" :key="index">
					<view class="u-btn-text" @tap.stop="btnClick(index)">{{ item.text }}</view>
				</view>
			</movable-view>
		</movable-area>
	</view>
...
export default {
	...
	methods: {
		...
		btnClick(index) {
			this.status = false;
			// this.index为点击的几个组件，index为点击某个组件的第几个按钮(options数组的索引)
			this.$emit('btnClick', this.index, index);
		},
		// movable-view元素移动事件
		change(e) {
