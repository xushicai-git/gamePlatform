...
export default {
	...
	methods: {
				...
				.then(res => {
					...
					res.tempFiles.map((val, index) => {
						...
						if (val.size > maxSize) {
							...
							this.showToast(this.getLang('超出允许的文件大小'));
						} else {
							if (maxCount <= lists.length) {
								this.$emit('on-exceed', val, this.lists, this.index);
								this.showToast(this.getLang('超出最大允许的文件个数'));
								return;
							}
							lists.push({
								url: val.path,
		...
		async uploadFile(index = 0) {
			...
			// 检查上传地址
			if (!this.action) {
				this.showToast(this.getLang('请配置上传地址'), true);
				return;
			}
			this.lists[index].error = false;
			this.uploading = true;
		...
		uploadError(index, err) {
			...
			this.showToast(this.getLang('上传失败，请重试'));
		},
		// 删除一个图片
		deleteItem(index) {
			uni.showModal({
				title: this.getLang('提示'),
				content: this.getLang('您确定要删除此项吗？'),
						...
						if(this.beforeRemove && typeof(this.beforeRemove) === 'function') {
							...
							if (!!beforeResponse && typeof beforeResponse.then === 'function') {
								await beforeResponse.then(res => {
									...
									this.showToast(this.getLang('已终止移除'));
								})
							} else if(beforeResponse === false) {
								// 返回false，终止删除
								this.showToast(this.getLang('已终止移除'));
		...
		handlerDeleteItem(index) {
			...
			this.showToast(this.getLang('移除成功'));
		},
		// 用户通过ref手动的形式，移除一张图片
		remove(index) {
			// 判断索引的合法范围
		...
		checkFileExt(file) {
			...
			if(!noArrowExt) this.showToast(this.getLang(`不允许选择此格式的文件`));
			return noArrowExt;
		}
	}
};
